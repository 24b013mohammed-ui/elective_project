...
def evaluate_model(model, test_loader, loss_fn, scalers_dict, device):
    ...
    logger.info(f"DEBUG: predictions_original type={type(predictions_original)}, len={len(np.atleast_1d(predictions_original))}")
    logger.info(f"DEBUG: targets_original type={type(targets_original)}, len={len(np.atleast_1d(targets_original))}")
    ...
    # Flatten in case of shape issues
    predictions_original_arr = predictions_original_arr.flatten()
    targets_original_arr = targets_original_arr.flatten()
    
    logger.info(f"DEBUG: After flattening - predictions_original_arr shape={predictions_original_arr.shape}, targets_original_arr shape={targets_original_arr.shape}")
    
    # Calculate errors with fallback safety
    try:
        errors_normalized = (targets - predictions).tolist()
        
        # Ensure same length and calculate errors
        if len(targets_original_arr) > 0 and len(predictions_original_arr) > 0:
            min_len = min(len(targets_original_arr), len(predictions_original_arr))
            errors_original = (targets_original_arr[:min_len] - predictions_original_arr[:min_len]).tolist()
        else:
            errors_original = []
        
        # Fallback if errors are empty
        if len(errors_original) == 0:
            logger.warning("ALERT: errors_original calculated as empty! Using normalized as fallback")
            errors_original = errors_normalized
            
        logger.info(f"SUCCESS: errors_original has {len(errors_original)} values (from {len(targets_original_arr)} targets, {len(predictions_original_arr)} predictions)")
        logger.info(f"Error stats - Normalized: {len(errors_normalized)} values, min={np.min(targets - predictions):.4f}, max={np.max(targets - predictions):.4f}, mean={np.mean(targets - predictions):.4f}")
        if len(errors_original) > 0:
            logger.info(f"Error stats - Original: {len(errors_original)} values, min={np.min(errors_original):.4f}, max={np.max(errors_original):.4f}, mean={np.mean(errors_original):.4f}")
    except Exception as e:
        logger.error(f"CRITICAL: Exception in error calculation: {e}", exc_info=True)
        errors_normalized = (targets - predictions).tolist()
        errors_original = errors_normalized
        logger.warning(f"Using {len(errors_original)} normalized errors as fallback")
    ...
    logger.info(f"Evaluation complete - Results contain {len(all_dates)} dates, {len(all_tickers)} tickers")
    logger.info(f"Error arrays: normalized={len(results['errors_normalized'])}, original={len(results['errors_original'])}")
    return results
